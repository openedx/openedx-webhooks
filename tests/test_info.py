...
def test_jira_info():
    ...
    assert (
        info["anotherorg"].mapping == "https://raw.githubusercontent.com/anotherorg/dot-github/HEAD/jira-mapping.yaml"
    )
...
def test_get_malformed_catalog_info(mocker, caplog):
    mocker.patch("openedx_webhooks.info.read_github_file", lambda *args, **kwargs: "bad: 'yaml")
...
def test_get_valid_catalog_info(mocker):
    mocker.patch("openedx_webhooks.info.read_github_file", lambda *args, **kwargs: "good: 'yaml'")
    info = get_catalog_info("foo")
